...
// UI Functions
        // Função para comparar datas (apenas dia/mês/ano)
        ...
        // Função para obter a classe CSS baseada na data
        function getDateClass(palletDate) {
            const today = new Date();
            const comparison = compareDates(palletDate, today);
            
            if (comparison < 0) {
                return 'past-date'; // Data passada - vermelho
            } else if (comparison > 0) {
                return 'future-date'; // Data futura - amarelo
            } else {
                return 'current-date'; // Data de hoje - azul (padrão)
            }
        }

        // Função melhorada para renderizar pallets
        function renderPallets() {
            ...
            if (pallets.length === 0) {
                palletList.innerHTML = `
                    <div class="empty-state">
                        ...
                        <p>Clique em "Adicionar Pallet de Teste" para criar alguns pallets de exemplo.</p>
                    </div>
                `;
                return;
            }
            ...
            pallets.forEach(pallet => {
                const totalCaixas = pallet.itens ? pallet.itens.reduce((sum, item) => sum + item.q_caixas, 0) : 0;
                const dateClass = getDateClass(pallet.data_criacao);
                
                const card = document.createElement('div');
                card.className = 'pallet-card';
                card.innerHTML = `
                    <div class="pallet-header ${dateClass}">
                    ...
                    <div class="pallet-body">
                        ...
                        <div class="pallet-info"><strong>Data:</strong> ${new Date(pallet.data_criacao).toLocaleDateString('pt-BR')}</div>
                    </div>
                    <div class="pallet-footer">
                        <span class="badge badge-success">${pallet.itens ? pallet.itens.length : 0} itens</span>
...
function renderItems() {
    ...
    currentPallet.itens.forEach(item => {
        ...
        itemElement.innerHTML = `
            ...
            <div class="item-row">
                <strong>Produtor:</strong> ${item.tex_produtor}
...
function fillPalletForm(pallet) {
    ...
    document.getElementById('n_imprimir').checked = pallet.cabecalho.n_imprimir || '';
...
function getPalletFormData() {
    ...
    return {
        cabecalho: {
            ...
            n_imprimir: document.getElementById('n_imprimir').checked
        },
...
function getItemFormData() {
    const produtorElement = document.getElementById('produtor');
    ...
    return {
        produtor: parseInt(produtorElement.value) || 0,
        tex_produtor: produtorElement.options[produtorElement.selectedIndex].text,
