...
class GvsSystem:
            ...
            if self.server:
                preferencias_do_chrome = {
                    'printing.print_preview_sticky_settings.appState': json.dumps({
                        "recentDestinations": [{"id": "ZDesigner GC420t (EPL) em GVS-PC351", "origin": "local"}],
                        "selectedDestinationId": "ZZDesigner GC420t (EPL) em GVS-PC351",
                        "version": 2,
                        "scalingType": 3,
                ...
                preferencias_do_chrome = {
                    'printing.print_preview_sticky_settings.appState': json.dumps({
                        ...
                        "recentDestinations": [{"id": "ZDesigner GC420t (EPL) em GVS-PC351", "origin": "local"}],
                        "selectedDestinationId": "ZDesigner GC420t (EPL) em GVS-PC351",
                        "version": 2,
                        "scalingType": 3,
                        "scaling": "100",
                        "copies": 2,
    ...
    def realizar_login(self):
            self.driver.get(self.url_login)
            self.driver.find_element(By.NAME,'username').send_keys(self.usuario)
...
class RegistroPalletManager(GvsSystem):
    ...
    def montagem_pallet(self, item: ItemPallet):
        ...
        sleep(1)
        self.selecionar_opcao_por_value(self.driver.find_element(By.NAME, 'produtor_id'), item.produtor)
        sleep(2)
        ...
        _input = self.driver.find_element(By.XPATH, '/html/body/span/span/span[1]/input')
        cor_input.send_keys(item.cor)
        cor_input.send_keys(Keys.ENTER)
        #f item.cor.lower() == "vermelha":
            #or_input.send_keys(Keys.ARROW_DOWN)
            #or_input.send_keys(Keys.ENTER)
       #else:
           #cor_input.send_keys(Keys.ENTER)
